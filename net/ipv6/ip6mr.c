...
{
	if (!mrt)
		return list_entry_rcu(&net->ipv6.mr_tables->next,
				      typeof(*mrt), list);
...
static int __net_init ip6mr_rules_init(struct net *net)
{
	...
	INIT_LIST_HEAD(&net->ipv6.mr_tables);
...
static void __net_exit ip6mr_rules_exit(struct net *net)
{
	...
	rtnl_lock();
	list_for_each_entry_safe(mrt, next, &net->ipv6.mr_tables, list) {
		list_del(&mrt->list);
		ip6mr_free_table(mrt);
	}
...
{
#ifdef CONFIG_IPV6_MROUTE_MULTIPLE_TABLES
	list_add_tail_rcu(&mrt->list, &net->ipv6.mr_tables);
#endif
}

struct mfc6_cache_cmp_arg ip6mr_mr_table_ops_cmparg_any = {
...
int __net_init ip6mr_notifier_init(struct net *net)
{
